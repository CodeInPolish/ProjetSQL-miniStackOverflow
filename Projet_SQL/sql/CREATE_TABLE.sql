	...
	***************************/
	...
	CREATE OR REPLACE VIEW ProjetSQL.get_user AS 
		SELECT u.user_id, u.pseudo, u.email, u.rank, u.reputation, u.last_vote_date FROM ProjetSQL.users u;

	CREATE OR REPLACE VIEW ProjetSQL.get_user_password AS
		SELECT user_id, password FROM ProjetSQL.users;

	CREATE OR REPLACE VIEW ProjetSQL.get_user_closed_view AS
		SELECT pseudo, closed FROM ProjetSQL.users;

	CREATE OR REPLACE VIEW ProjetSQL.unknow_user AS
		SELECT user_id, pseudo FROM ProjetSQL.users;
...
WHERE qt.user_id = u1.user_id AND qt.edit_date IS NULL AND qt.closed = FALSE ORDER BY qt.date DESC
...
CREATE OR REPLACE FUNCTION ProjetSQL.connectUser(VARCHAR(20)) RETURNS INTEGER AS $$
DECLARE
	_pseudo ALIAS FOR $1;
	return_id INTEGER;
BEGIN
...
CREATE OR REPLACE FUNCTION ProjetSQL.CreateUser(VARCHAR(50), VARCHAR(60), VARCHAR(30)) RETURNS INTEGER AS $$
DECLARE 
	_email ALIAS FOR $1;
